...
class Config:
    ...
    def get_aws_region() -> str:
        # Boto3 automatically gets the region from the Lambda execution environment
...
def create_response(status_code: int, body: Dict[str, Any]) -> Dict[str, Any]:
    """Create standardized HTTP response with proper CORS headers"""
    return {
        'statusCode': status_code,
        'headers': {
            'Content-Type': 'application/json',
            'Access-Control-Allow-Origin': '*',  # Allow all origins
            'Access-Control-Allow-Methods': 'OPTIONS,POST,GET',  # Essential methods
            'Access-Control-Allow-Headers': 'Content-Type,X-Amz-Date,Authorization,X-Api-Key,X-Amz-Security-Token,x-api-key,Origin,Accept',  # Include all necessary headers
...
def lambda_handler(event: Dict[str, Any], context: Any) -> Dict[str, Any]:
    ...
    try:
        # Handle preflight OPTIONS request for CORS - works for both REST and HTTP APIs
        if (event.get('httpMethod') == 'OPTIONS' or 
            event.get('requestContext', {}).get('http', {}).get('method') == 'OPTIONS'):
            logger.info("Handling OPTIONS preflight request")
            return create_response(200, {})
        
        # Parse the request body - handle both REST and HTTP API formats
...
if __name__ == "__main__":
    # Test with both REST and HTTP API event formats
    test_events = [
        # REST API format
        {
            'httpMethod': 'POST',
...
#             'Access-Control-Allow-Origin': '*',  # Allow all origins
#             'Access-Control-Allow-Methods': 'OPTIONS,POST,GET',  # Essential methods
#             'Access-Control-Allow-Headers': 'Content-Type,X-Amz-Date,Authorization,X-Api-Key,X-Amz-Security-Token,x-api-key',  # Include x-api-key
#             'Access-Control-Max-Age': '86400'
#         },
#         'body': json.dumps(body)
#     }
...
#     try:
#         # Handle preflight OPTIONS request for CORS - HTTP API format
#         if event.get('requestContext', {}).get('http', {}).get('method') == 'OPTIONS':
#             logger.info("Handling OPTIONS preflight request for HTTP API")
#             return create_response(200, {})
        
#         # Parse the request body - HTTP API format
#         if 'body' in event and event['body']:
#             if isinstance(event['body'], str):
#                 body = json.loads(event['body'])
#             else:
...
# # For local testing
# if __name__ == "__main__":
#     # Test with HTTP API event format
#     test_event = {
#         'version': '2.0',
#         'routeKey': 'POST /chat',
#         'rawPath': '/dev/chat',
...
#             'Access-Control-Allow-Headers': 'Content-Type,X-Amz-Date,Authorization,X-Api-Key,X-Amz-Security-Token,Origin,Accept,X-Requested-With',
#             'Access-Control-Allow-Credentials': 'true',
#             'Access-Control-Max-Age': '86400'
#         },
#         'body': json.dumps(body)
